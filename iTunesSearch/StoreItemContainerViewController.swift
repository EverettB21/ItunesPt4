...
class StoreItemContainerViewController: UIViewController, UISearchResultsUpdating {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let tableVC = segue.destination as? StoreItemListTableViewController {
            configureTableViewDataSource(tableVC.tableView)
        }
        if let collectionVC = segue.destination as? StoreItemCollectionViewController {
            configureCollectionViewDataSource(collectionVC.collectionView)
        }
    }
